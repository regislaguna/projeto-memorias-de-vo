    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-end z-50">
      <div className="w-full max-w-md bg-white h-full flex flex-col">
        ...
        <div className="p-4 border-b flex justify-between items-center">
          <h2 className="text-2xl font-serif">Meu Carrinho</h2>
          <button onClick={onFechar} className="text-2xl">&times;</button>
        </div>
            ...
            itens.map(item => (
              <div key={item.produto.id} className="flex items-center justify-between mb-4">
                ...
                <div className="flex items-center gap-2">
                  <button onClick={() => onAtualizarQuantidade(item.produto.id, item.quantidade - 1)} className="px-2 border rounded">-</button>
                  <span>{item.quantidade}</span>
                  <button onClick={() => onAtualizarQuantidade(item.produto.id, item.quantidade + 1)} className="px-2 border rounded">+</button>
                  <button onClick={() => onRemoverItem(item.produto.id)} className="text-red-500 ml-2">Remover</button>
                </div>
              </div>
            ))
          )}
